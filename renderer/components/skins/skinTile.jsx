export default function SkinTile({ skinImage, skinName, extraClasses, onClick, isOwned, contentTier }) {
  return (
    <div 
      id='skin-tile'
      className= {
        'box-border shadow-lg hover:shadow-2xl border-2 border-separate relative flex flex-col h-24 text-center items-center justify-center bg-maincolor-lightest bg-opacity-60 py-4 rounded hover:bg-opacity-100 cursor-pointer transition-colors ease-in duration-100 mb-2 overflow-hidden '
        +
        (extraClasses != '' ? extraClasses : 'border-maincolor-lightest')
      }
      onClick={onClick}
    >
